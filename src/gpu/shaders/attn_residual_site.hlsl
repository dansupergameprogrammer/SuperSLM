...
void main(uint3 gtid : SV_GroupThreadID)
{
    ...
    {
        int selected_code = branch_selected ? LayerScratch.Load<int>(o_codes_off + (uint)i * 4u)
                                            : SeqState.Load<int>((uint)i * 4u);
        int nonselected_code = branch_selected ? SeqState.Load<int>((uint)i * 4u)
                                               : LayerScratch.Load<int>(o_codes_off + (uint)i * 4u);
