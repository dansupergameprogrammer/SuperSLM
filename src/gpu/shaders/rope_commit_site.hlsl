...
//     twelve of 256 threads doing all the work at the real 1.5B tier, in one group. The real
//     independent work item is a (head, rotation pair): num_attention_heads * head_dim/2 of them,
//     each rotating one disjoint (x,y) pair. Flattened over that and gridded.
//
// (2) The K COMMIT phase moved to its own dispatch (rope_commit_site.hlsl). The
//     DeviceMemoryBarrierWithGroupSync that used to separate stage from commit is GROUP-scoped, so
//     it orders only the 256 threads of one group -- going multi-group under it would be unsound,
//     which is precisely why the split is a second dispatch rather than a wider one. The global UAV
//     barrier bind_and_dispatch issues between the two dispatches is strictly stronger than the
//     group barrier it replaces: it orders EVERY thread of the stage dispatch before EVERY thread
//     of the commit dispatch, which is the read-before-write property D-SLM2993's own shape exists
//     to establish. It is also what makes this site time-slice invariant -- the two dispatches may
//     land in different command lists submitted in different slices with a fence between them and
