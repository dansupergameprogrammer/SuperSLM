...
void main(uint3 gtid : SV_GroupThreadID)
{
    ...
    // T-2105 (Laplace, DISPOSABLE): the two GEMM calls that used to stand here moved to
    // kv_proj_gemm_site.hlsl, a multi-group coalesced dispatch issued immediately before this one.
    // Both wide rows (K at WorkScratch base 0, V at WIDE_B) are already published to this dispatch
    // by the global UAV barrier bind_and_dispatch issues after every dispatch -- strictly stronger
    // than the DeviceMemoryBarrierWithGroupSync that stood here, which only ordered the 256 threads
    // of this one group. Everything below this line is unchanged: the fused K+V bias-precedence
    // check, the landing rescale, and the clamp counter are still one group's own cooperative work
    // over the kv_hidden_size row, which is what this site's own header comment correctly says
    // cannot be split across groups.
