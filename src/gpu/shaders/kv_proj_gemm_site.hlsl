...
// merges). kv_proj's own two GEMM steps (K and V), split out of kv_proj_site.hlsl into their own
// multi-group coalesced dispatch.
//
// WHY THIS SPLIT IS AVAILABLE, against kv_proj_site.hlsl's own header comment saying it is not.
// That comment names the real obstacle correctly -- the fused K+V bias-precedence check
// (gKFailed/gVFailed) and the landing-rescale clamp counter (gTotalClamps) ARE genuine cross-thread
// cooperative reductions over the whole kv_hidden_size row, and splitting THEM across groups would
// need atomics or reordered accumulation. But it names the remedy in the same breath: "without
// either atomics or A SECOND DISPATCH". A second dispatch is exactly what this file is. The GEMM
// step alone has no cross-thread dependency at all (each output channel's own reduction is
// independent of every other), so it moves out to a multi-group dispatch and the guard/landing pass
// stays behind, single-group, over the 256-element kv_hidden_size row it was always cheap on. The
// UAV barrier `bind_and_dispatch` already issues after every dispatch is what publishes the wide
// rows from this dispatch to the next -- strictly stronger synchronisation than the
// DeviceMemoryBarrierWithGroupSync the fused version relied on, and the reason this split is also
// time-slice invariant: the two dispatches may be submitted in different command lists, in
// different execution slices, with a fence between them, and the bytes do not change.
//
// K AND V IN ONE DISPATCH. The dispatch covers 2*kv_hidden_size logical output channels: channel
// c < kv_hidden_size is K's own channel c (weights off_kw, wide row at WorkScratch base 0), and
// c >= kv_hidden_size is V's own channel c - kv_hidden_size (weights off_vw, wide row at
// WorkScratch's own WIDE_B base). Both halves land in exactly the byte ranges the fused version's
// own two GemmParallelGpu calls wrote, so kv_proj_site.hlsl's own downstream code reads the
// identical values from the identical addresses.
//
// The accumulation is GemmCoalescedGpuAt's own int64 groupshared tree -- see site_common.hlsli's
// own header comment for why a tree over integer addition returns the identical int64 the serial
// loop does, and why that is the reduction shape this substrate already ships (RmsSumSqParallelGpu).
