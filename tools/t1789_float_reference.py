#!/usr/bin/env python3
"""T-1789: a copy of T-1788's own `tools/t1788_float_prescore_reference.py` (read in full before
reuse), extended by exactly ONE addition: it ALSO dumps the float model's own per-layer INPUT
hidden state (the residual stream entering each checkpoint layer, i.e. the tensor
`input_layernorm` consumes) at EVERY prompt position, to `--hidden-dump`. This is a read-only
observation of a value the EXISTING proven oracle loop (`oracle_recompute_attn`) already carries
as its own `hidden_states` local at every step -- not a new computation path. The value is
captured BEFORE the layer runs, so it is exactly the quantity the engine's own `h[]`
(RmsNorm's input, t1789_attention_probe.cpp Dump 1) corresponds to on the float side.
Everything else -- the two-path oracle discipline, the float32-throughout load, the independence
from the engine's own construction -- is unchanged from T-1788's copy, whose own header follows.

T-1788: unmodified copy of T-1787's own `tools/t1787_float_prescore_reference.py` (branch
`claude/t1787-raw-score-divergence`, read in full before reuse), renamed only so this ticket's
worktree does not depend on another seat's branch -- every property T-1787/T-1786/T-1778 already
established (independence from the engine's own int8 construction; genuine float32 compute; the
two-path oracle cross-check; the pre-softmax-score self-check) applies unchanged, on this ticket's
own 3 new held-out prompts.
...
def oracle_recompute_attn(model, input_ids, device):
    ...
    # T-1789 addition: per-(layer, position) input hidden state at checkpoint layers.
    oracle_hidden: dict[tuple[int, int], "torch.Tensor"] = {}
            ...
            for t in range(input_ids.shape[1]):
                ...
                for i, layer in enumerate(model.model.layers[:n_layers]):
                    if i in CHECKPOINT_LAYERS:
                        # T-1789 addition: the residual stream ENTERING this layer, at this
                        # position -- captured before the layer consumes it.
                        oracle_hidden[(i, t)] = hidden_states.detach()[0, 0, :].float().clone()
    ...
    return oracle_captured, oracle_scores, oracle_hidden
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--hidden-dump", required=True,
                        help="path to write the per-(layer,position) input-hidden-state dump (T-1789 addition)")
    ...
    oracle_captured, oracle_scores, oracle_hidden = oracle_recompute_attn(model, input_ids, device)
    ...
    # T-1789 addition: per-(layer, position) input hidden state dump.
    hidden_path = Path(args.hidden_dump)
    hidden_path.parent.mkdir(parents=True, exist_ok=True)
    hidden_size = model.config.hidden_size
    hrows = sorted(oracle_hidden.keys())
    expected = {(l, t) for l in CHECKPOINT_LAYERS for t in range(prompt_len)}
    missing = expected - set(hrows)
    if missing:
        raise AssertionError(f"hidden-state capture incomplete: {len(missing)} missing (layer,pos) rows")
    with open(hidden_path, "w", encoding="ascii") as f:
        f.write(f"{len(hrows)} {hidden_size}\n")
        for (layer, pos) in hrows:
            vals = oracle_hidden[(layer, pos)].double().tolist()
            f.write(f"{layer} {pos} " + " ".join(f"{v!r}" for v in vals) + "\n")
    print(f"hidden_dump_written: {len(hrows)} (layer,position) rows, hidden_size={hidden_size} "
          f"-> {hidden_path}")
    return 0
