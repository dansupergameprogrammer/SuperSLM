/* t2139_sslm_g5_ref.h -- T-2139 (Brunel): a verbatim promoted copy of
 * tests/t2130-g5-red-suite/sslm_g5.h @ curie/t2130-g5-red-suite@760b12b -- byte-identical to
 * `git show 760b12b:tests/t2130-g5-red-suite/sslm_g5.h`, confirmed against the branch's own
 * status-enum body at that commit (unchanged since, through curie/t2130-g5-red-suite@c885a98).
 * This file exists because tests/t2130-g5-red-suite/ is not part of this branch's own history
 * (curie/t2138-abi-red-suite + main, per this ticket's own worktree instructions) and is not
 * suite-owned content this build may edit read-only-in-place -- Gate A/C (this directory) need
 * a real copy of sslm_g5.h to compare this design's own header against, per design Sec9's own
 * literal construction ("include sslm_g5.h once, unmodified, at its real names"). Regenerate
 * from the cited commit if that file changes; never hand-patched.
 *
 * The original file's own header comment follows unchanged below (its own citations, own
 * commit references) -- only this banner and the include-guard rename (SSLM_G5_H ->
 * SUPERSLM_T2139_SSLM_G5_REF_H, so this copy and a real tests/t2130-g5-red-suite/sslm_g5.h can
 * both be included in the same translation unit without a guard collision) are new.
 *
 * ORIGINAL HEADER COMMENT, UNCHANGED:
 *
 ...
 */
#ifndef SUPERSLM_T2139_SSLM_G5_REF_H
#define SUPERSLM_T2139_SSLM_G5_REF_H
...
#endif /* SUPERSLM_T2139_SSLM_G5_REF_H */
