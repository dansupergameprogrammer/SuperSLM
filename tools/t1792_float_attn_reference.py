#!/usr/bin/env python3
"""T-1792: a copy of T-1778's own `tools/t1778_float_attn_reference.py` (branch
claude/t1778-float-attn-reference, read in full before reuse; verified D-SLM1234-1240), with
exactly ONE change: `CHECKPOINT_LAYERS` below is widened from the six-layer sample
{0,1,2,9,18,27} to ALL 28 layers {0..27}. This does not touch the reference's own arithmetic in
any way -- the model's forward pass already computes and cross-checks every layer on every
step (`oracle_recompute_attn` loops `for i, layer in enumerate(model.model.layers[:n_layers])`
unconditionally); the CHECKPOINT_LAYERS tuple only controls which already-computed layers are
written to the dump file. T-1792's own commission requires all 28 layers measured, not a sample.
Every independence and precision property T-1778 established (no import of the engine's own
integer construction; genuine float32 compute, TF32 disabled; the two-path oracle cross-check)
is unchanged and is re-verified by execution on this ticket's own run, not inherited by citation
alone (`StandardsDocument.md` SS5.4).

T-1778's own header follows unchanged:
...
CHECKPOINT_LAYERS = tuple(range(28))  # T-1792: all 28 layers (was the 6-layer T-1778/T-1789 sample)
