...
if [ "$STAGE" = "all" ] || [ "$STAGE" = "engine-free" ]; then
  for ID in $IDS; do
    ...
    ./out/t1836_decode_residual_probe.exe "$MODEL" "$TOK" "$P" "$ID" --dump-dir "$DUMPDIR" \
      --max-new "$MAXNEW" --suffix _free > "$DUMPDIR/${ID}_free_engine.log" 2>&1
    tail -4 "$DUMPDIR/${ID}_free_engine.log"
  done
fi
...
if [ "$STAGE" = "all" ] || [ "$STAGE" = "float" ]; then
  for ID in $IDS; do
    ...
    python -u tools/t1836_float_decode_reference.py "$U" --system "$SYS" --max-new "$MAXNEW" \
      --dump "$DUMPDIR/${ID}_float.npy" > "$DUMPDIR/${ID}_float.log" 2>&1
    tail -4 "$DUMPDIR/${ID}_float.log"
  done
fi
...
if [ "$STAGE" = "all" ] || [ "$STAGE" = "engine-forced" ]; then
  for ID in $IDS; do
    U="$(prompt_for "$ID")"
    P=$'<|im_start|>system\n'"$SYS"$'<|im_end|>\n<|im_start|>user\n'"$U"$'<|im_end|>\n<|im_start|>assistant\n'
    GEN="$(python -c "import json,sys;m=json.load(open(sys.argv[1]));print(','.join(str(t) for t in m['generated']))" "$DUMPDIR/${ID}_float.npy.meta.json")"
    echo "=== engine teacher-forced $ID ==="
    ./out/t1836_decode_residual_probe.exe "$MODEL" "$TOK" "$P" "$ID" --dump-dir "$DUMPDIR" \
      --max-new "$MAXNEW" --suffix _forced --force-tokens "$GEN" \
      > "$DUMPDIR/${ID}_forced_engine.log" 2>&1
    tail -5 "$DUMPDIR/${ID}_forced_engine.log"
  done
fi
