// t1772_layer0_candidates_probe.cpp -- T-1772: test the two remaining named candidates for
// layer 0's ~20x unclamped-element error elevation that T-1768 (D-SLM1167, section 7d) left
// untested after ruling out pure-z coarsening, polynomial fit-quality, and distribution
// flatness:
//
//   (A) "Rounding in the final Q15 probability division `(e[k]<<15)/total` interacting with
//       e[k] values that are themselves tiny integers when M is tiny" -- tested below by
//       reconstructing production's own per-element evaluated value `e[k]` via the SAME public
//       primitives SoftmaxRowQ15 calls internally (ShiftByMax, IExpConstruct, IExpEvaluate),
//       then dividing that SAME e[k] by the row's own real total in DOUBLE precision instead of
//       production's integer division -- isolating what the truncating integer divide itself
//       contributes, per element, from what e[k]'s own value already carries.
//   (B) "A genuine discretization/aliasing effect specific to `q_p` having only 6 representable
//       values ... distinct from the relative-error sweep in section 7b" -- tested below by a
//       causal ablation: for the SAME row of raw integer scores (untouched), a synthetic
//       (q_ln2', q_b', q_c') triple is re-derived by calling the SAME production function
//       (`IExpScaleConstants`) with the row's real `sm.m` and an artificially finer `sm.e`
//       (T-1768 section 4's own finding: `shift_ln2 = 92 + sm.e`, so a more negative `sm.e`
//       yields a smaller `shift_ln2` and hence a larger `q_ln2`) -- then `SoftmaxRowQ15` and
//       `IdealProbsQ15` (the production kernel and T-1763's own ideal reconstruction,
//       unmodified) are called on the UNCHANGED scores with this synthetic triple. If
//       resolution/aliasing drives the elevation, widening q_ln2 alone, holding the row fixed,
//       should shrink the real/ideal gap toward the 0.9-1.2 baseline measured at every other
//       checkpointed layer.
//
// Both tests operate at the ELEMENT level on the SAME rows the disagreement was measured on
// (not on layer-level aggregates) -- T-1769's own debunking of the clip-crowding attribution
// (D-SLM1158/1159) found that two elevated aggregates sharing an upstream cause is not evidence
// one causes the other; this file does not repeat that mistake.
//
// Extends T-1768's own self-checked instrument (tools/t1768_qln2_derivation_probe.cpp, branch
// claude/t1768-qln2-trace@61a09a1, read not owned -- this is an independent copy, not a commit
// to that branch), which itself extends T-1763's own instrument
// (tools/t1763_layer0_diagnosis_probe.cpp, branch claude/t1763-layer0-diagnosis@10cd1e2, also
// read not owned). T-1768's own header (below) describes its own additions; this comment does
// not repeat them.
//
// Original T-1768 header, unchanged:
//
// T-1768: establish where layer 0's derived `q_ln2 = 6`
...
// T-1772 candidate A: per-UNCLAMPED-element isolation of the final Q15 division's own
// contribution. `real_q15`/`ideal_q15` are the same values ElemZDiag already captures;
// `hybrid_q15` divides production's own per-element evaluated value `real_e[k]` by the row's
// own real integer total in DOUBLE precision instead of production's truncating integer divide
// -- so `delta_real_vs_hybrid` is what the integer division itself contributes (candidate A,
// if this is comparable in magnitude to the full real/ideal delta, the division is implicated;
// if it is small, the division is exonerated and the error lives in `e[k]` itself) and
// `delta_hybrid_vs_ideal` is what remains once the division's own contribution is removed.
struct HybridDiag {
	uint32_t layer = 0;
	int64_t z = 0;
	int64_t real_q15 = 0, hybrid_q15 = 0, ideal_q15 = 0;
	int64_t delta_real_vs_hybrid = 0;
	int64_t delta_hybrid_vs_ideal = 0;
};

// T-1772 candidate B: causal ablation result for one (layer, kv_head, e_offset) triple -- the
// SAME row of raw scores, reprocessed through a synthetic, finer (q_ln2', q_b', q_c') triple
// derived by the SAME production function (IExpScaleConstants) from the row's real `sm.m` and
// an artificially adjusted `sm.e`. `n_unclamped`/`sum_abs_delta` are this triple's own
// unclamped-population real/ideal Q15 disagreement (its own clip window, not the original's),
// aggregated per row so the report can average across rows/prompts without keeping every
// element.
struct AblationDiag {
	uint32_t layer = 0;
	int64_t e_offset = 0;
	int64_t synth_q_ln2 = 0;
	int64_t n_unclamped = 0;
...
SslmForwardStatus ManualRunOneLayer(SequenceLayerState& seq, const LayerWeights& lw,
                                     ...
                                     std::vector<HybridDiag>* out_hybrid,
                                     std::vector<AblationDiag>* out_ablation) {
	...
	std::vector<int64_t> ctx_wide(hidden_size), ctx_wide_ideal(hidden_size);
	{
		...
		// T-1772 candidate B: the row's own real combined carried scale, cached per kv_head so
		// the ablation below can re-derive a synthetic triple from the SAME sm.m without
		// recomputing CombineCarriedScale.
		std::vector<CarriedScale> khead_sm(num_key_value_heads);
		for (size_t h = 0; h < num_heads; ++h) {
			...
			// T-1772 candidate A: reconstruct production's own per-element evaluated value
			// `e[k]` via the SAME public primitives SoftmaxRowQ15 calls internally
			// (intmath.cpp:845-878: ShiftByMax, then per-element IExpConstruct/IExpEvaluate),
			// then divide that SAME e[k] by the row's own real total in DOUBLE precision --
			// isolating the final integer division's own contribution from what e[k] itself
			// already carries. Self-checked per element: `(real_e[k]<<15)/max(real_total,1)`
			// (the SAME integer division SoftmaxRowQ15 performs) must reproduce `probs[k]`
			// exactly.
			if (out_hybrid != nullptr) {
				const int64_t q_ln2_h = khead_q_ln2[kv_head];
				const int64_t q_b_h = khead_q_b[kv_head];
				const int64_t q_c_h = khead_q_c[kv_head];
				std::vector<int64_t> shifted_real(width);
				ShiftByMax(scores.data(), width, shifted_real.data());
				std::vector<int64_t> real_e(width);
				int64_t real_total = 0;
				for (size_t k = 0; k < width; ++k) {
					IExpConstruction c;
					const IExpDomain d = IExpConstruct(shifted_real[k], q_ln2_h, q_b_h, q_c_h, &c);
					if (d != IExpDomain::kOk && d != IExpDomain::kNotRepresentable) {
						std::fprintf(stderr,
						             "FAILED at stage=hybrid_reconstruction: layer=%u head=%zu "
						             "k=%zu IExpConstruct rejected an element SoftmaxRowQ15 "
						             "already accepted\n",
						             l, h, k);
						std::exit(1);
					}
					real_e[k] = IExpEvaluate(c);
					real_total += real_e[k];
				}
				const int64_t denom = real_total > 0 ? real_total : 1;
				for (size_t k = 0; k < width; ++k) {
					const int64_t reconstructed = (real_e[k] << kProbFracBits) / denom;
					if (reconstructed != probs[k]) {
						std::fprintf(stderr,
						             "FAILED at stage=hybrid_self_check: layer=%u head=%zu k=%zu "
						             "reconstructed=%lld probs=%lld -- reuse invalid, aborting\n",
						             l, h, k, static_cast<long long>(reconstructed),
						             static_cast<long long>(probs[k]));
						std::exit(1);
					}
				}
				const double real_total_d = static_cast<double>(real_total);
				const int64_t clip = -kIExpClipN * q_ln2_h;
				int64_t mx2 = scores[0];
				for (size_t k = 1; k < width; ++k) mx2 = std::max(mx2, scores[k]);
				for (size_t k = 0; k < width; ++k) {
					const int64_t shifted_k = scores[k] - mx2;
					if (shifted_k < clip) continue;  // clamped -- excluded, T-1769's own split
					const int64_t clipped_k = std::max<int64_t>(shifted_k, clip);
					const double p_hybrid_d =
					    std::round(static_cast<double>(real_e[k]) / real_total_d * 32768.0);
					const int64_t p_hybrid = static_cast<int64_t>(p_hybrid_d);
					HybridDiag hd;
					hd.layer = l;
					hd.z = (-clipped_k) / q_ln2_h;
					hd.real_q15 = probs[k];
					hd.hybrid_q15 = p_hybrid;
					hd.ideal_q15 = ideal_probs_q15[k];
					hd.delta_real_vs_hybrid = std::abs(probs[k] - p_hybrid);
					hd.delta_hybrid_vs_ideal = std::abs(p_hybrid - ideal_probs_q15[k]);
					out_hybrid->push_back(hd);
				}
			}

			// T-1772 candidate B: causal ablation. For the SAME row of raw scores (untouched),
			// re-derive a synthetic, finer (q_ln2', q_b', q_c') triple via the SAME production
			// function (IExpScaleConstants), from the row's real sm.m and sm.e shifted finer by
			// `e_offset` bits (T-1768 section 4: shift_ln2 = 92+sm.e, so a more negative sm.e
			// yields a larger q_ln2). Then call the SAME production kernel (SoftmaxRowQ15) and
			// T-1763's own IdealProbsQ15 on the unchanged scores with this synthetic triple.
			if (out_ablation != nullptr) {
				const CarriedScale& sm_real = khead_sm[kv_head];
				for (const int64_t e_offset : {int64_t{-4}, int64_t{-8}, int64_t{-12}}) {
					const int64_t synth_e = sm_real.e + e_offset;
					int64_t sq_ln2 = 0, sq_b = 0, sq_c = 0;
					const IExpScaleDomain sdom =
					    IExpScaleConstants(sm_real.m, synth_e, kIExpLn2Q, 30, kIExpBQ, 30, kIExpCaQ,
					                       30, &sq_ln2, &sq_b, &sq_c);
					if (sdom != IExpScaleDomain::kOk) continue;  // out of domain -- skipped, not guessed
					if (CheckSoftmaxRowWidthDomain(sq_b, sq_c, width) != SslmForwardStatus::Ok) {
						continue;
					}
					std::vector<int64_t> probs_synth(width), ideal_synth;
					const bool wf =
					    SoftmaxRowQ15(scores.data(), width, sq_ln2, sq_b, sq_c, probs_synth.data());
					if (!wf) continue;  // ill-formed at this synthetic offset -- skipped
					IdealProbsQ15(scores, sq_ln2, &ideal_synth);
					const int64_t clip_s = -kIExpClipN * sq_ln2;
					int64_t mx3 = scores[0];
					for (size_t k = 1; k < width; ++k) mx3 = std::max(mx3, scores[k]);
					AblationDiag ad;
					ad.layer = l;
					ad.e_offset = e_offset;
					ad.synth_q_ln2 = sq_ln2;
					for (size_t k = 0; k < width; ++k) {
						if ((scores[k] - mx3) < clip_s) continue;
						++ad.n_unclamped;
						ad.sum_abs_delta += std::abs(probs_synth[k] - ideal_synth[k]);
					}
					out_ablation->push_back(ad);
				}
			}
...
int main(int argc, char** argv) {
	...
	std::vector<HybridDiag> all_hybrid;
	std::vector<AblationDiag> all_ablation;
	...
	for (uint32_t step = 0; step < num_hidden_layers; ++step) {
		...
		if (is_checkpoint) {
			...
			std::vector<HybridDiag> step_hybrid;
			std::vector<AblationDiag> step_ablation;
			const SslmForwardStatus mst =
			    ManualRunOneLayer(manual_seq, layers[step], hidden_size, head_dim, num_kv_heads,
			                      ...
			                      &step_chain, &step_elem_z, &step_peak, &step_hybrid,
			                      &step_ablation);
			...
			for (auto& hd : step_hybrid) all_hybrid.push_back(hd);
			for (auto& ad : step_ablation) all_ablation.push_back(ad);
		} else {
			const SslmForwardStatus pst =
			    RunLayerLoop(trace_seq, layers.data(), num_hidden_layers, /*layer_budget=*/1,
			                 hidden_size, head_dim, num_kv_heads, intermediate_size, context_cap,
	...
	// -- Report T-1772 candidate A: division-rounding isolation. `real_vs_hybrid` is the
	// integer division's own contribution; `hybrid_vs_ideal` is what remains once that
	// contribution is removed. Both are self-checked per element (reconstructed real_e/total
	// must reproduce production's own probs[k] exactly, or the run aborts before this prints).
	std::printf("\n[T-1772 candidate A: final Q15 division isolation, per checkpoint layer, "
	            "unclamped population only]\n");
	for (uint32_t cl : checkpoint_layers) {
		int64_t n = 0, sum_rh = 0, sum_hi = 0, max_rh = 0, sum_ri = 0;
		for (const auto& hd : all_hybrid) {
			if (hd.layer != cl) continue;
			++n;
			sum_rh += hd.delta_real_vs_hybrid;
			sum_hi += hd.delta_hybrid_vs_ideal;
			sum_ri += std::abs(hd.real_q15 - hd.ideal_q15);
			max_rh = std::max(max_rh, hd.delta_real_vs_hybrid);
		}
		if (n == 0) continue;
		std::printf("  layer=%u: n=%lld mean|real-hybrid|=%.4f (division's own contribution, "
		            "max=%lld) mean|hybrid-ideal|=%.4f (residual after division removed) "
		            "mean|real-ideal|=%.4f (full delta, cross-check vs item 5)\n",
		            cl, static_cast<long long>(n), static_cast<double>(sum_rh) / static_cast<double>(n),
		            static_cast<long long>(max_rh), static_cast<double>(sum_hi) / static_cast<double>(n),
		            static_cast<double>(sum_ri) / static_cast<double>(n));
	}

	// -- Report T-1772 candidate B: causal ablation. The SAME rows, reprocessed through a
	// synthetic finer q_ln2' via the SAME production function and kernel. If resolution drives
	// the elevation, mean|delta| at increasingly negative e_offset should fall from layer 0's
	// baseline (~21, item 5) toward the 0.9-1.2 band measured at every other checkpointed
	// layer.
	std::printf("\n[T-1772 candidate B: causal ablation, per checkpoint layer and e_offset -- "
	            "SAME rows, synthetic finer q_ln2']\n");
	for (uint32_t cl : checkpoint_layers) {
		for (const int64_t e_offset : {int64_t{-4}, int64_t{-8}, int64_t{-12}}) {
			int64_t n = 0, sum_delta = 0, qln2_min = -1, qln2_max = -1;
			for (const auto& ad : all_ablation) {
				if (ad.layer != cl || ad.e_offset != e_offset) continue;
				n += ad.n_unclamped;
				sum_delta += ad.sum_abs_delta;
				if (qln2_min < 0 || ad.synth_q_ln2 < qln2_min) qln2_min = ad.synth_q_ln2;
				if (ad.synth_q_ln2 > qln2_max) qln2_max = ad.synth_q_ln2;
			}
			if (n == 0) {
				std::printf("  layer=%u e_offset=%lld: no in-domain rows (skipped)\n", cl,
				            static_cast<long long>(e_offset));
				continue;
			}
			std::printf("  layer=%u e_offset=%lld: synth_q_ln2=[%lld,%lld] n=%lld "
			            "mean|delta|=%.4f\n",
			            cl, static_cast<long long>(e_offset), static_cast<long long>(qln2_min),
			            static_cast<long long>(qln2_max), static_cast<long long>(n),
			            static_cast<double>(sum_delta) / static_cast<double>(n));
		}
	}
