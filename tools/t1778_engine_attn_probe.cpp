// t1778_engine_attn_probe.cpp -- T-1778: dump the compiled engine's REAL Q15 attention
// probabilities (SoftmaxRowQ15's own output, the exact per-head row production feeds into
// the V-matmul), self-checked bit-for-bit against production, for the last prompt token at
// a fixed set of checkpoint layers -- for comparison against an INDEPENDENT float reference
// (tools/t1778_float_attn_reference.py) that shares nothing with this engine's integer
// construction (IExpScaleConstants, IdealProbsQ15, the i-exp path).
//
// This file is a copy of tools/t1763_layer0_diagnosis_probe.cpp's own per-layer manual
// replay (branch claude/t1763-layer0-diagnosis@10cd1e2, itself extending T-1762's
// tools/t1762_attn_ctx_probe.cpp, branch brunel/t1762-attn-ctx-pin@f378049) -- both read in
// full, neither committed to, neither modified. This probe strips T-1763's downstream
// propagation (o_proj/mlp/residual, both real and ideal arms) and its IdealProbsQ15 arm
// entirely -- T-1778's question is answered by grading the REAL arm against an EXTERNAL
// float reference, not against another construction sharing IExpScaleConstants -- and adds
// nothing IExpScaleConstants-derived to what gets dumped: only `width` (a loop bound, not a
// derived integer constant) and the raw Q15 `probs` array (SoftmaxRowQ15's own output) are
// written out.
//
// Self-check identical in kind to T-1762/T-1763/T-1755: at every checkpoint layer, the
// manual replay's own resulting hidden_codes/hidden_scale is compared bit-for-bit against
// PRODUCTION's own RunLayerLoop(layer_budget=1) call for that layer, from the same
// pre-layer state. `probs` itself is validated transitively, the same argument T-1762 uses
// for ctx_codes: it is computed by the identical compiled SoftmaxRowQ15 call, on the
// identical inputs, at the identical point in the call sequence production's own
// RunLayerLoop uses, and the replay's LATER output (hidden_codes, a deterministic function
// of probs with no other input that could coincidentally mask a wrong probs) is proven
...
// -- verbatim copies of forward_sites.cpp's internal-linkage helpers (T-1762/T-1763's own
// justification: ProjectAndFunnel and ApplyBiasReconcileRow are internal-linkage in
// forward_sites.cpp, re-confirmed at this worktree's HEAD; bodies copied verbatim, calling
// only public-header primitives, never a re-derivation of their arithmetic). --
...
// One captured (layer, head) softmax row: the REAL arm's Q15 probabilities exactly as
// SoftmaxRowQ15 emits them, nothing derived from IExpScaleConstants added to the dump
// beyond the row width (a loop bound).
...
// head's REAL Q15 probs (CapturedProbRow), and continuing the REAL arm's computation
// (unchanged from RunLayerLoop's own body) so `seq`/`workspace` advance identically to
// production -- required for the self-check below.
SslmForwardStatus ManualRunOneLayer(SequenceLayerState& seq, const LayerWeights& lw,
	...
	// what the self-check validates, which transitively validates `probs` above. ---
...
int main(int argc, char** argv) {
	...
	// Same checkpoint-layer convention as T-1762/T-1763/T-1768: {0,9,18,27} is D-SLM1135-
	// 1137's own cell; {1,2} added (T-1763's own extension) to distinguish "layer 0
	...
	// Dump: layer head width probs[width], one row per (layer,head), text format for easy
	// cross-language re-verification (matches this campaign's own scratch-tool convention).
	...
	for (const auto& row : all_rows) {
		f << row.layer << " " << row.head << " " << row.width;
