...
def test_real_tree_decode_sticky_tag_status_set_is_the_named_sixteen():
    with open(chk.SUPERSLM_GPU_CPP, "r", encoding="utf-8") as f:
        gpu_text = f.read()
    ...
    assert names == {
        ...
        "ResidualReconciliationScaleOutOfDomain",
...
_BELOW_TWENTY = ["zero"] + _ONES_WORDS + ["ten", "eleven", "twelve", "thirteen", "fourteen",
                                         "fifteen", "sixteen", "seventeen", "eighteen", "nineteen"]
_TENS_WORDS = ["twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty", "ninety"]


def _english(n):
    # Written here from the English number system, independently of the checker's own table.
    if n < 20:
        return _BELOW_TWENTY[n]
    tens, ones = divmod(n, 10)
    return _TENS_WORDS[tens - 2] + ("" if ones == 0 else "-" + _ONES_WORDS[ones - 1])


@pytest.mark.parametrize("n", range(100))
def test_every_number_word_zero_to_ninety_nine_parses(n):
    # TE-437: the table named "twenty" and every hyphenated compound but no round ten above twenty,
    # so gpu_port.h's "forty" (TE-435) raised. Each value 0-99 in both anchor positions; with the
    # round-tens line removed from the checker, n = 30, 40, .., 90 fail here.
    word = _english(n)
    text = (f"// the recording-window catch, {word}\n// paths in all, and\n"
            f"// alike, {word} paths' own destination in\n// total\n")
    assert chk.parse_lwuws_path_counts(text) == (n, n)
...
def test_the_real_tree_lwuws_before_count_is_thirty_three():
    ...
    # (GpuGemmGroupArithmeticError's/std::runtime_error's, covering its own recording body), PLUS
    # (T-2189 finding 6, D-SLM3695) that same function's own SECOND pair of catch clauses
    # (std::bad_alloc's/std::runtime_error's, covering its own previously-uncovered tail), PLUS
    # (T-2195 remedy S1's class sweep, Claude/Poirot/1381076-t2195-t2189-closing-confirmation.md
    # Observation O1) RunLayerLoopGpuSubmit's own IDENTICAL second pair of catch clauses
    # (std::bad_alloc's/std::runtime_error's, covering its own previously-uncontained tail, mirroring
    # the sibling) -- eight catch CLAUSES total then, on top of the thirteen ladder/device-capability
    # returns. T-2192's own T1/T2 remedy (Claude/Poirot/896553e-t2192-t2189-closing-confirmation.md)
    # split each function's own tail runtime_error clause into three RETURNS instead of one (a
    # honest-terminal early return when a second Close() attempt also fails, plus two textually
    # distinct copies of the original ternary) -- now true of BOTH functions' own tail clauses, not
    # only the sub-chunk primitive's: 13 + 12 = 25, gpu_port.h's own prior prose. T-2568 (S1,
    # Claude/Poirot/66626ef-t2567-trackb-confirmation.md) then added ONE new catch clause to EACH
    # of RunLayerLoopGpuSubmit and SubmitOneSubChunkToFullDepthForG5Bridge
    # (GpuLayerWeightsContractError's own, one literal return each, PackLayerWeightsBytes' required-
    # pointer refusal) -- ten catch clauses total now, +2 returns: 13 + 14 = 27, matching
    # gpu_port.h's own then-current prose. The ladder/device-capability term reads from
    # PrepareGpuLayerLoopChunkOpenState's own body (shared, not duplicated); the catch term sums
    ...
    # (`SameModelFastPathAllowed`, S1, same ticket) is deliberately a top-level function rather
...
def test_the_real_tree_lwuws_after_count_is_seven():
    ...
    # T-2192's own T1/T2 remedy does not touch any after-decision return -- unchanged at six.
    # T-2568's own new catch clauses sit inside the SAME two functions' own recording-window
    # try blocks the existing catch clauses already do -- BEFORE the residency decision, not
    # after -- so this count is unchanged too.
    # CORRECTED 2026-09-25 (TE-437; TE-435's class closure, 1831cd0): RunLayerLoopGpuFinish gains a
    # final catch (...) returning ClassifyForwardFault, its fifth return (the null-token return, the
    # pure-poll Ok, DecodeStickyTag, the std::exception clause, the catch-all), read at 91779ad:
    # 1 + 1 + 5 = 7.
    with open(chk.SUPERSLM_GPU_CPP, "r", encoding="utf-8") as f:
        gpu_text = f.read()
...
def test_the_real_tree_lwuws_total_is_forty():
    ...
    # CORRECTED 2026-09-25 (TE-437; TE-435's finish catch-all): the before-count stays 33 (neither
    # RunLayerLoopGpuSubmit nor SubmitOneSubChunkToFullDepthForG5Bridge changed), the after-count moves
