...
def _rotate_wide_pair_row(seg, cos_row, sin_row):
    """T-1891 (T-1822 §29.4/§30, D-SLM2305) -- Option G's fused pre-landing K rotation,
    the reference-side mirror of the engine's `RopeApplyPairWide` (spike copy,
    `forward_sites.cpp`). Applies `rope.rope_apply_pair` -- the SAME Q2.30 rotation and
    SAME single C3 (ties-away-from-zero) rounding the narrow post-landing rotation
    already uses -- pairwise over `seg`, the WIDE (unlanded, pre-`landing_rescale_vec`)
    per-head K accumulator segment, at this token's absolute position's table row.
    ...
    mirror the engine's S128 facility against. This is why no "wide" sibling of
    ...
    was landed before, per D-SLM2305's construction ("one `LandingRescale` per head on
    the existing static per-head K landing constants").
    """
    pairs = len(seg) // 2
    out = list(seg)
...
def _forward_dynamic_vec_layers(model, tokens, cache=None, trace=None,
    ...
    for layer in range(cfg.num_hidden_layers):
        ...
        for t in range(steps):
            ...
            for head in range(num_kv_heads):
                ...
                if option_g_fused_k_landing:
                    # T-1891/D-SLM2305: rotate the WIDE pre-landing segment pairwise at
                    # this token's absolute position, THEN land once -- K carries one
                    # int8 boundary instead of two (§29.4's construction). V and Q are
        ...
        if option_g_fused_k_landing:
            # T-1891/D-SLM2305: K is already rotated at landing time (above) -- the
...
def forward_dynamic_vec(model, tokens, cache=None, trace=None,
    ...
    T-1891 (T-1822 §29.4/§30, D-SLM2305): `option_g_fused_k_landing`, default False
    (unchanged land-then-rotate order, matching the engine's own flag default) — True
    selects Option G's fused pre-landing K rotation (§29.4's construction). This
    parameter exists only on this whole-token entry point; `MicroStepState`/
    `begin_token`/`decode_step`'s own resumable micro-step path is unmodified and stays
    on the False (unfused) order in every case — this spike's dual-run parity harness
    (T-1891 gate G3) drives whole-token forwards only, never a resumed micro-step, so
    threading the flag through the resumable path is out of this spike's scope and is
    named in its casebook's untested list rather than silently extended here.
    """
...
class MicroStepState:
    ...
    def __init__(self, model, tokens, cache=None, trace=None):
        self._stepper = _forward_dynamic_vec_layers(model, tokens, cache=cache, trace=trace)
        self.layers_done = 0
        self.logits = None
        self.complete = False


def begin_token(model, tokens, cache=None, trace=None) -> MicroStepState:
