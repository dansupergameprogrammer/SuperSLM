...
// T-2234 (SuperSLM 1.2.1): the SAME hermetic fixture with NO damped-greedy opt-in (no DGC1
// section, no feature bit) -- the pre-1.2 artifact shape. Generated by the S8 generator's
// second output (tools/_t2199_s8_synthetic_full_model_fixture.py argv[2]). Empty means "not
// supplied" -- the workspace-layout cells SKIP rather than assert against nothing.
static std::string g_model_plain_path;
...
inline void ParseFixtureArgs(int argc, char** argv) {
	for (int i = 1; i < argc; ++i) {
		...
		else if (const char* v = take("--modelplain=")) g_model_plain_path = v;
	}
}
